...
class IAMAT(object):
    def __init__(self, id, lat, lng, time=None):
        ...
        self.lat = lat
        self.lng = lng
        self.time = time if time is not None else currentPosixTime()
...
class AT(IAMAT):
    def __init__(self, clientId, lat, lng, clientTime, serverId, serverTime=None, fromId=None):
        ...
        self.serverTime = serverTime if serverTime is not None else currentPosixTime()
        self.fromId = fromId
    ...
    def __str__(self):
        return 'AT ' + self.serverId + ' ' + floatToStr(self.timeDiff) + \
          ...
          ' ' + floatToStr(self.lat, True) + floatToStr(self.lng, True) + \
          ' ' + floatToStr(self.time) + \
          ((' ' + self.fromId) if self.fromId is not None else '') + '\n'
