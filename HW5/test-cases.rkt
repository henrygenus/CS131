...
(define all-tests
  (list (test-case 12 12 12) ; # 1
        ...
        (test-case '(cons (cons a b) (cons b c)) ;10
        ...
        (test-case '(quote (a b)) '(quote (a c)) '(if % '(a b) '(a c))) ;14
        (test-case '(quoth (a b)) '(quoth (a c)) '(quoth (a (if % b c)))) ;15
        (test-case '(if x y z) '(if x z z)  '(if x (if % y z) z)) ;16
        ...
        (test-case ''((λ (a) a) c) ''((lambda (b) b) d) ;21
                   '(if % '((λ (a) a) c) '((lambda (b) b) d)))
        (test-case '(+ #f ((λ (a b) (f a b)) 1 2)) ;22
                   '(+ #t ((lambda (a c) (f a c)) 1 2))
                   '(+ (not %) ((λ (a b!c) (f a b!c)) 1 2)))
        (test-case '((λ (a b) (f a b)) 1 2) '((λ (a b) (f b a)) 1 2) ;23
                   '((λ (a b) (f (if % a b) (if % b a))) 1 2))
        (test-case '((λ (a b) (f a b)) 1 2) '((λ (a c) (f c a)) 1 2) ;24
                   '((λ (a b!c) (f (if % a b!c) (if % b!c a))) 1 2))
        (test-case '((lambda (lambda) (+ lambda if (f lambda))) 3) ;25
                   '((lambda (if) (+ if if (f λ))) 3)
                   '((lambda (lambda!if)
                       (+ lambda!if (if % if lambda!if) (f (if % lambda!if λ)))) 3))
        (test-case '((lambda (a) (eq? a ((λ (a b) ((λ (a b) (a b)) b a)) ;26
...
(define extra-tests
  (list (test-case '(λ a a) '(λ b b) '(λ a!b a!b)) ;1
        (test-case '(λ a a) '(λ (a) a) '(if % (λ a a) (λ (a) a))) ;2
        (test-case '(cons a lambda) '(cons a λ) '(cons a (if % lambda λ))) ;3
        (test-case '(lambda (a) a) '(lambda (b) b) '(lambda (a!b) a!b)) ;4
        (test-case '(lambda (a) b) '(cons (c) b) '(if % (lambda (a) b) (cons (c) b))) ;5
        (test-case '((λ (if) (+ if 1)) 3) '((lambda (fi) (+ fi 1)) 3) ;6
              '((λ (if!fi) (+ if!fi 1)) 3))
        (test-case '(lambda (lambda) lambda) '(λ (λ) λ) '(λ (lambda!λ) lambda!λ)) ;7
        (test-case ''lambda '(quote λ) '(if % 'lambda 'λ)) ;8
        (test-case '(lambda (a b) a) '(λ (b) b) '(if % (lambda (a b) a) (λ (b) b))) ;9
        (test-case '(λ (a b) (lambda (b) b)) '(lambda (b) (λ (b) b)) ;10
                      '(if % (λ (a b) (lambda (b) b)) (lambda (b) (λ (b) b))))
        (test-case '(λ (let) (let ((x 1)) x)) '(lambda (let) (let ((y 1)) y)) ;11
                      '(λ (let) (let (((if % x y) 1)) (if % x y))))
        (test-case '(λ (x) ((λ (x) x) x)) '(λ (y) ((λ (x) y) x)) ;12
